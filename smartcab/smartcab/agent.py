...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            #self.epsilon = math.exp(-self.alpha*self.it)
           # self.epsilon = 1/math.exp(self.it)
            self.epsilon = 0.999 ** (self.it*10)
            #self.epsilon=math.cos(self.alpha*self.it)
           # self.epsilon = math.fabs(math.cos(self.alpha*self.it))
            self.it += 1
        print(self.epsilon)
        return None
    ...
    def build_state(self):
        ...
        #   If it is not, create a dictionary in the Q-table for the current
        #   'state'
    ...
    def get_maxQ(self, state):
        ...
        KeyMax = max(self.Q[state], key=self.Q[state].get) 
        print(self.Q[state][KeyMax])
        return self.Q[state][KeyMax]
    ...
    def choose_action(self, state):
        ...
        if not self.learning or random.random() <= self.epsilon:
            ...
            v_max = self.get_maxQ(state)
            actions = [k for k,v in self.Q[state].items() if v == v_max]
...
def run():
    ...
    # Flags:
    #   update_delay - continuous time (in seconds) between actions, default is
    #   2.0 seconds
